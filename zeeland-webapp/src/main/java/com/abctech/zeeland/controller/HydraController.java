...
public class HydraController {
    ...
    public String denied(Model model) {
        hydraClient.justTryToPingMyself(propertiesManager.getGlobalProperty("hydra.shall.ping.zeeland.on.this.url"));
        ApiProperty hydraUrl = propertiesManager.getGlobalProperty("hydra.director.url.prefix");
        if (hydraUrl == null) {
            throw new ZeelandException("You must configure up hydra.director.url.prefix in wp.xml");
        }
        model.addAttribute("redirectUrl", hydraUrl.getValue() + "/service.html?name=Zeeland");
        return "hydra/noauth";
    }
